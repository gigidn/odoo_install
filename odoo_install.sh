...
# (could be used for other version too) but not tested
...
OE_PORT="8069"
#Choose the Odoo version which you want to install. For example: 10.0, 9.0, 8.0, 7.0 or saas-6.
...
while getopts ":h :i :u: :e :p: :v: :d:" opt; do
    case "$opt" in
    h|\?)
        show_help
        exit 0
    ...
    u)  OE_USER=$OPTARG
        ;;
    e)  IS_ENTERPRISE="True"
        ;;
    p)  OE_SUPERADMIN=$OPTARG
        ;;
    v)  OE_VERSION=$OPTARG
        ;;
    d)  OE_HOME=$OPTARG
...
echo "interactive=$INTERACRIVE, odoo_usr='$OE_USER', is_ent='$IS_ENTERPRISE', password='$OE_SUPERADMIN' "

if [ "$INTERACRIVE" -eq "1" ]; then
    echo -n "Odoo system user [odoo]: "
    read OE_USER

    if [ -z "$OE_USER" ]; then
      OE_USER="odoo"
    fi
    echo "utente scelto: $OE_USER"

    echo -n "Admin Password [admin]: "
    read OE_SUPERADMIN

    if [ -z "$OE_SUPERADMIN" ]; then
      OE_SUPERADMIN="admin"
    fi
    echo "password scelta: $OE_SUPERADMIN"
fi
...
sudo pip install gdata psycogreen ofxparse XlsxWriter codicefiscale unicodecsv
...
sudo su root -c "echo 'data_dir = $OE_HOME/filestore' >> /etc/${OE_CONFIG}.conf"
if [  $IS_ENTERPRISE = "True" ]; then
    sudo su root -c "echo 'addons_path=$OE_HOME/enterprise/addons,$OE_HOME_EXT/addons,$OE_HOME/custom/addons' >> /etc/${OE_CONFIG}.conf"
else
